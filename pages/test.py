...
EXPECTED_FIELDS = [
    ...
    "Total Experience", "Most Recent Job Title", "Highest Qualification",
    "Key Skills"
]
...
def extract_text_from_docx(file):
    try:
        ...
        st.error(f"Error extracting text from DOCX file {file.name}: {e}")
        return ""
...
def extract_info_with_groq(resume_text, timeout=90):
    ...
    try:
        ...
        st.error(f"Groq API call failed (HTTPError): {e}")
        return None
    except Exception as e:
...
uploaded_files = st.file_uploader(
    "Upload Resume Files (.pdf, .docx, .png, .jpg, .jpeg)",
